...
public class FinalCandidateSafetyIntegrationTest
{
    ...
    public void futureProviderCannotLeakMembersContentToUnknownOrF2p()
    {
        ...
        assertOnlyFallback(evaluate(account(Membership.UNKNOWN, 70, 70, 70, 70,
                70, 70, 70, 70), members));
        assertOnlyFallback(evaluate(account(Membership.F2P, 70, 70, 70, 70,
                70, 70, 70, 70), members));
        assertEquals("future:members", evaluate(account(Membership.P2P,
    ...
    {
        AccountSnapshot oneDefence = account(Membership.P2P,
        ...
        assertFalse(evaluate(oneDefence, candidate("future:mining",
                Safety.skill(false, Skill.MINING))).isEmpty());

        AccountSnapshot ambiguousRangeTank = account(Membership.P2P,
        ...
        assertFalse(evaluate(ambiguousRangeTank, candidate("future:harmless",
                Safety.harmless(false))).isEmpty());

        AccountSnapshot combatOnly = account(Membership.P2P,
                70, 70, 70, 70, 70, 70, 80, 1);
        assertOnlyFallback(evaluate(combatOnly, candidate("future:farming",
                Safety.skill(false, Skill.FARMING))));

        for (AccountSnapshot restricted : java.util.Arrays.asList(
                account(Membership.P2P, 1, 1, 60, 1, 1, 1, 70, 50),
                account(Membership.P2P, 1, 1, 1, 1, 1, 1, 10, 50),
                account(Membership.P2P, 1, 1, 1, 1, 20, 1, 10, 50),
                account(Membership.P2P, 1, 1, 1, 20, 1, 20, 10, 50)))
        {
    ...
    {
        AccountSnapshot defencePure = account(Membership.P2P,
                1, 1, 60, 1, 43, 1, 70, 50);
    ...
    public void actualUpgradeProviderAllowsDefencePureFighterTorso()
    {
        ...
        StrategyEngine engine = TestFixtures.strategyEngine(null, null, null,
    ...
    public void blockedPvmNeverAppearsInFinalQueue()
    {
        Recommendation blocked = new Recommendation("pvm:obor", "Do Obor",
                ...
                guidance(), Safety.potentiallyIrreversible(true));
        assertOnlyFallback(evaluate(account(Membership.P2P, 70, 70, 70, 70,
                70, 70, 70, 70), blocked));
    }
            ...
            int magic, int hitpoints, int nonCombat)
    {
        ...
        return new AccountSnapshot("Safety", 0L, 0, "Main", membership, 1, 1000, 0L, levels, xp);
    }
}
