...
public class UniversalActionRecipeResolver
{
            ...
            MembershipStatus membership)
    {
        ...
        switch (action.getSkill())
        {
            ...
            case FIREMAKING: return lower.contains("log")
                    ? recipe(Text.get(937), count, name, 1)
                    : unknown(Text.get(938));
            case PRAYER: return contains(lower, "bones", "ashes", "head")
                    ? recipe(Text.get(939), count, name, 1)
                    : unknown(Text.get(900));
            case RUNECRAFT: return recipe(Text.get(901), count,
            ...
            case FARMING: return lower.endsWith(" tree")
                    ? recipe(Text.get(930), count,
                    name.substring(0, name.length() - 5).trim() + " sapling", 1)
                    : unknown(Text.get(929));
            case MAGIC: return lower.endsWith(" curse")
                    ? recipe(Text.get(933), count, "Earth rune", 3,
                    "Water rune", 2, "Body rune", 1)
                    : unknown(Text.get(934));
            case HERBLORE: return unknown(Text.get(925));
            case CONSTRUCTION: return unknown(Text.get(928));
    ...
    private static UniversalActionRecipe cooking(String name, String lower, int n)
    {
        if (contains(lower, "jug of wine") || lower.equals("wine"))
            return recipe(Text.get(903), n, "Grapes", 1, "Jug of water", 1);
    ...
    private static UniversalActionRecipe crafting(String name, String lower, int n)
    {
        ...
        if (lower.contains("bird house"))
            return recipe(Text.get(911), n, wood(name, "Logs"), 1);
        ...
        if (jewellery(lower))
        {
            ...
            return gem == null ? recipe(Text.get(1265), n, bar, 1)
                    : recipe(Text.get(1265), n, bar, 1, capitalize(gem), 1);
        }
        if (contains(lower, "battlestaff", "battlestave"))
        {
            String element = first(lower, "air", "water", "earth", "fire");
            if (element != null) return recipe(Text.get(912), n, "Battlestaff", 1,
    ...
    private static UniversalActionRecipe fletching(String name, String lower, int n)
    {
        ...
        if (lower.equals("headless arrow") || lower.equals("headless arrows"))
            return recipe(Text.get(915), n, "Arrow shaft", 1, "Feather", 1);
        ...
        if (lower.contains("broad arrow")) return recipe(Text.get(917), n,
                "Headless arrow", 1, "Broad arrowhead", 1);
        if (projectile(lower, "arrow")) return recipe(Text.get(918), n,
                "Headless arrow", 1, metal + " arrowhead", 1);
        if (projectile(lower, "javelin")) return recipe(Text.get(919), n,
                "Javelin shaft", 1, metal + " javelin head", 1);
        if (basicBolt(lower)) return recipe(Text.get(920), n,
