...
| Sailing starter route and trials | [Pandemonium](https://oldschool.runescape.wiki/w/Pandemonium), [Port task](https://oldschool.runescape.wiki/w/Port_task), [Barracuda Trials](https://oldschool.runescape.wiki/w/Barracuda_Trials), pinned RuneLite `VarPlayerID` Sailing constants | Pandemonium proves the starter raft, Captain's log access, Port Sarim/Pandemonium courier loop, and sea-charting access. RuneLite varplayers prove owned boat slots, active port tasks, and completed Tempor/Jubbly/Gwenith trials. Compass names the starter courier loop and each trial's launch point, but keeps unmapped chart checkboxes and fitted boat components as explicit unknown/preparation evidence rather than inventing live readiness. |
